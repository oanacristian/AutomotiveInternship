...
#define MSG_BUF_SIZE  4    /* Msg Buffer Size. (CAN 2.0AB: 2 hdr +  2 data= 4 words) */
  ...
  CAN0->RAMn[ 4*MSG_BUF_SIZE + 1] = 0x15540000; /* Msg Buf 4, word 1: Standard ID = 0x555 */                                                /* PRIO = 0: CANFD not used */
...
void FLEXCAN0_transmit_msg(void) { /* Assumption:  Message buffer CODE is INACTIVE */
  ...
  CAN0->RAMn[ 0*MSG_BUF_SIZE + 2] = 0x00000000;//0xA5112233; /* MB0 word 2: data word 0 */
  CAN0->RAMn[ 0*MSG_BUF_SIZE + 3] = 0x00000001;//0x44556677; /* MB0 word 3: data word 1 */
...
void FLEXCAN0_transmit_msg_ushort(unsigned short value) { /* Assumption:  Message buffer CODE is INACTIVE */
  ...
  CAN0->RAMn[ 0*MSG_BUF_SIZE + 3] = 0x00000000;//0x44556677; /* MB0 word 3: data word 1 */
  CAN0->RAMn[ 0*MSG_BUF_SIZE + 1] = 0x14440000; /* MB0 word 1: Tx msg with STD ID 0x511 */
  CAN0->RAMn[ 0*MSG_BUF_SIZE + 0] = 0x0C400000 | 8 <<CAN_WMBn_CS_DLC_SHIFT; /* MB0 word 0: */
                                                /* EDL,BRS,ESI=0: CANFD not used */
                                                /* CODE=0xC: Activate msg buf to transmit */
...
unsigned short FLEXCAN0_receive_msg(void) {  /* Receive msg from ID 0x556 using msg buffer 4 */
  uint8_t j;
  uint32_t dummy;
