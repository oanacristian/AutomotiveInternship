...
void PORT_init (void) {
  ...
  PTD->PDDR |= 1<<PTD10;
  PORTD->PCR[PTD10] =  0x00000100;  /* Port D10: PWM*/
}
...
int main(void)
{
  ...
  for(;;) {
  		// we map the distance to an interval: 3 means back-up with full speed, 97 means full speed forward, and in the middle nothing happens
		...
		converted = map_int(distance, 3, 97, 280, 565);
		if(distance<20)
		  {
